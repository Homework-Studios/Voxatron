...
public abstract class TowerSelector extends Element implements Runnable {
    ...
    public void update() {
        // hover check
        collisions = new Raylib.Rectangle().x(position.x - size.x / 2).y(position.y - size.y / 2 - hoverOffset).width(size.x).height(size.y + UiUtils.getHeightPercent(5));
        ...
        float maxScroll = (towers.length - 3) * (towerWidth + UiUtils.getWidthPercent(1));
        ...
        float targetHoverOffset = hover ? UiUtils.getHeightPercent(20) : 0;
